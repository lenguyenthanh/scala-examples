...
class SubmarineBenchmarks:
  ...
  def simpleIOWithThrowable(bh: Blackhole) =
    1.to(size)
      .toList
      .traverse_(_ => IO(Blackhole.consumeCPU(cpuTokens)) *> simpleIOWithThrowableImpl)
      .unsafeRunSync()
...
def simpleIOWithThrowableImpl: IO[String] =
  IO.raiseError(Error2)
    .as("nope")
    .handleErrorWith:
      case Error2 => IO.pure("error")
