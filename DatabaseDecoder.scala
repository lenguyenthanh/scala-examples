...
/**
 * implement a partial function: List[String] => List[Pgn]
 * pars line by line
 * use uncons, TagParsers and MovesParser
  */
object Hello extends IOApp.Simple:
  val stream = Stream.emits(1 to 10)

  def run =
    stream.mapAccumulate(0)
      { case (acc, n) => if predicate(acc + n) then (0, acc + n) else (acc + n, 0)}
      .collect{case (s, x) if x != 0=> x}
      .evalTap(IO.println).compile.drain
  ...
  def run1 =
    for
      c <- Files[IO].readAll(Path("lichess_db_standard_rated_2013-01.pgn.zst"))
